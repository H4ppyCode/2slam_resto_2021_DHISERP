...
//        $stmt->bindValue(':mdpU', $mdpUCrypt, PDO::PARAM_STR);
    ...
    public static function delete(int $idU): bool {
        $ok = false;
        try {
            $requete = "DELETE FROM aimer WHERE idU = :idU;
                        DELETE FROM critiquer WHERE idU = :idU;
                        DELETE FROM preferer WHERE idU = :idU;
                        DELETE FROM utilisateur WHERE idU = :idU
                        ";
            $stmt = Bdd::getConnexion()->prepare($requete);
            $stmt->bindValue(':idU', $idU, PDO::PARAM_INT);
            $ok = $stmt->execute();
